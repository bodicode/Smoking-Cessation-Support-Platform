...
function statusBadge(status: string) {
    ...
    return (
        <span className={`ml-2 px-3 py-1 rounded-full text-xs font-bold shadow-sm ${colors[status] || "bg-gray-100 text-gray-600"}`}>
            {status}
        </span>
    );
}
...
export default function CustomStages() {
    ...
    if (!plans.length) return <div className="text-center text-red-500 py-20">Không tìm thấy kế hoạch nào.</div>;
            ...
            {plans.map((plan, i) => {
                ...
                return (
                    ...
                    >
                        ...
                        <div className="text-gray-700 mb-1"><b>Lý do bắt đầu:</b> {plan.reason}</div>
                        <div className="flex flex-wrap gap-4 mb-2 text-gray-600">
                            <div><b>Ngày bắt đầu:</b> {new Date(plan.start_date).toLocaleDateString()}</div>
                            <div><b>Ngày mục tiêu:</b> {new Date(plan.target_date).toLocaleDateString()}</div>
                        </div>
                        ...
                        <div className="mb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex-1 relative h-5 rounded-full bg-sky-100 overflow-hidden">
                                    <div className="absolute left-0 top-0 h-full bg-gradient-to-r from-sky-400 to-green-400 transition-all" style={{ width: `${plan.completion_percentage ?? 0}%` }}></div>
                                </div>
                        ...
                        <div className="mb-4 flex items-center gap-4">
                            <label htmlFor={`status-${plan.id}`} className="block font-bold mb-1">Trạng thái kế hoạch:</label>
                            <select
                                id={`status-${plan.id}`}
                                className="w-44 border px-4 py-2 rounded-xl text-base"
                                value={plan.status}
                                onChange={async (e) => {
                                    const newStatus = e.target.value;
                                    try {
                                        await updateCessationPlan({ id: plan.id, status: newStatus as CessationPlanStatus });
                                    ...
                                    <li key={stage.id} className="bg-white border rounded-xl p-4 shadow flex flex-col gap-2">
                                        <form onSubmit={(e) => handleCustomSave(plan.id, e)} className="flex flex-col gap-2">
                                            ...
                                            <input
                                                type="date"
                                                name="start_date"
                                                value={customForm[plan.id]?.start_date?.slice(0, 10) || ""}
                                                onChange={(e) => handleChange(plan.id, e)}
                                                className="border rounded px-2 py-2 text-base"
                                            />
                                            <input
                                                type="date"
                                                name="end_date"
                                                value={customForm[plan.id]?.end_date?.slice(0, 10) || ""}
                                                onChange={(e) => handleChange(plan.id, e)}
                                                className="border rounded px-2 py-2 text-base"
                                            />
                                            <textarea
                                    ...
                                    >
                                        <div>
                                            ...
                                            <div className="text-sky-700 text-lg mt-2">
                                                <b>Giai đoạn {stage.stage_order}: {stage.title}</b>
                                            </div>
                                            ...
                                            <div className="mt-2 flex items-center gap-2">
                                                <label htmlFor={`stage-status-${stage.id}`} className="text-sm">
                                                    <b className="text-gray-500">Trạng thái:</b>
                                                </label>
                                                <select
                                                    id={`stage-status-${stage.id}`}
                                                    className="border px-2 py-1 rounded-xl text-base"
                                                    value={stage?.status}
                                                    onChange={async (e) => {
                                                        const newStatus = e.target.value;
                                                        try {
