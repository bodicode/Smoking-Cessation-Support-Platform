...
import {
  ...
  CheckCircle, Award as AwardIcon, Heart, Shield
} from "lucide-react";
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <motion.div
            className="bg-gradient-to-r from-sky-50 to-green-50 rounded-2xl shadow-md p-6 border-l-8 border-sky-400"
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
          ...
          >
            <h3 className="text-xl font-bold text-sky-700 mb-4 flex items-center gap-2">
              ...
              Tại sao chọn Coach này?
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex items-start gap-3">
                <Shield className="w-5 h-5 text-sky-600 mt-1" />
                <div>
                  <p className="font-semibold text-gray-800">Kinh nghiệm chuyên sâu</p>
                  <p className="text-gray-600 text-sm">Với {coachProfile.experience_years} năm kinh nghiệm trong lĩnh vực tư vấn bỏ thuốc</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <Target className="w-5 h-5 text-green-600 mt-1" />
                <div>
                  <p className="font-semibold text-gray-800">Tỷ lệ thành công cao</p>
                  <p className="text-gray-600 text-sm">{coachProfile.success_rate.toFixed(1)}% khách hàng đã bỏ thuốc thành công</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <Users className="w-5 h-5 text-purple-600 mt-1" />
                <div>
                  <p className="font-semibold text-gray-800">Được tin tưởng</p>
                  <p className="text-gray-600 text-sm">Đã hỗ trợ {coachProfile.total_clients} khách hàng trên hành trình bỏ thuốc</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <Star className="w-5 h-5 text-yellow-600 mt-1" />
                <div>
                  <p className="font-semibold text-gray-800">Đánh giá xuất sắc</p>
                  <p className="text-gray-600 text-sm">Được đánh giá {coachProfile.average_rating.toFixed(1)}/5 sao từ khách hàng</p>
                </div>
              </div>
            </div>
          </motion.div>
        ...
        <div className="space-y-6">
          ...
          <motion.div
            className="bg-gradient-to-r from-sky-500 to-green-400 rounded-2xl shadow-md p-6 text-white"
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: 0.4 }}
          >
            <h3 className="text-lg font-bold mb-3">Bắt đầu hành trình với Coach</h3>
            <p className="text-sky-100 mb-4">
              Hãy để Coach {coachProfile.approach_description} đồng hành cùng bạn trên con đường bỏ thuốc
            </p>
            <Link
              href="/template"
              className="inline-block bg-white text-sky-600 font-semibold py-2 px-4 rounded-lg hover:bg-gray-50 transition-colors"
            >
              Xem các kế hoạch
            </Link>
          </motion.div>
